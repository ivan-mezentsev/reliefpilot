...
- Authorized Telegram users can:
  ...
  - use `/pending`, `/blockers`, `/errors`, `/resume`, `/status`, `/diff`, `/patch`, `/summary`, `/mode`, and `/commands` as phone-first quick actions
...
- `/blockers` — show recent blocking or failed remote items for the active session
- `/errors` — show recent failure items only, when you want failure-focused triage
